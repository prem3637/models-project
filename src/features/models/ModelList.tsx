...
export const ModelList: React.FC = () => {
  ...
  const columns = [
        ...
        return (
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => handleOpenDetails(model)}>
            ...
            <div className="flex flex-col min-w-0">
              <span className="text-xs font-bold text-slate-800 group-hover:text-accent-600 transition-colors truncate">
                {model.name}
              </span>
              <span className="text-[10px] text-slate-400 truncate font-medium">{model.email}</span>
            </div>
          </div>
        );
      }
    ...
    columnHelper.accessor('age', {
      header: 'Age',
      cell: info => <span className="text-xs text-slate-700 font-semibold">{info.getValue()} yrs</span>
    }),
    columnHelper.accessor('height', {
      header: 'Height',
      cell: info => <span className="text-xs text-slate-700 font-semibold">{info.getValue()} cm</span>
    }),
    columnHelper.accessor('weight', {
      header: 'Weight',
      cell: info => <span className="text-xs text-slate-700 font-medium">{info.getValue()} kg</span>
    }),
        ...
        return (
          <div className="flex items-center gap-1.5">
            <button
              ...
              title="View Profile"
              className="p-1.5 rounded bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 transition-colors border border-slate-200"
            >
            ...
            {ability.can('delete', 'Model') && (
              <button
                ...
                className="p-1.5 rounded bg-slate-100 hover:bg-red-50 text-slate-650 hover:text-red-600 transition-colors border border-slate-200"
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col gap-5">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
          <div className="flex flex-col gap-1">
            <h2 className="text-base font-extrabold text-slate-900">Smart Filtering</h2>
            <p className="text-xs text-slate-400">Search and segment the roster in real-time</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
            <div className="relative w-full sm:w-64 md:w-72">
              <input
                ...
                className="w-full pl-9 pr-3.5 py-2 bg-white border border-slate-300 rounded-lg text-xs text-slate-800 placeholder-slate-400 focus:border-accent-500 focus:ring-4 focus:ring-accent-500/20 outline-none transition-all"
              />
        ...
        {/* Range sliders */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 border-t border-slate-100 pt-4">
          {/* Age range */}
          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center text-[10px] font-bold text-slate-500 uppercase tracking-wider">
              <span>Age Range</span>
              <span className="text-accent-600 font-bold">{filters.minAge} - {filters.maxAge} yrs</span>
            </div>
          ...
          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center text-[10px] font-bold text-slate-500 uppercase tracking-wider">
              ...
              <span className="text-accent-600 font-bold">{filters.minHeight} - {filters.maxHeight} cm</span>
            </div>
            <div className="flex items-center gap-4">
              <input
                type="range"
            ...
            <table className="w-full text-left text-xs border-collapse min-w-[1150px]">
              <thead>
                {table.getHeaderGroups().map(headerGroup => (
                  <tr key={headerGroup.id} className="border-b border-slate-200 bg-slate-50 text-slate-500 font-bold uppercase tracking-wider">
