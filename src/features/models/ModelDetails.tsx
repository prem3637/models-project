      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 flex flex-col md:flex-row gap-6 shadow-sm">
            <div className="w-32 h-32 md:w-40 md:h-40 rounded-2xl overflow-hidden shrink-0 border border-slate-200 shadow-inner">
              <img
                src={model.imageUrl}
                alt={model.name}
            ...
            <div className="flex-1 flex flex-col justify-center gap-3">
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center gap-3 flex-wrap">
                  <h2 className="text-xl md:text-2xl font-extrabold text-slate-900">{model.name}</h2>
                  <span
                ...
                <span className="text-xs font-semibold text-accent-600 uppercase tracking-widest">{model.category} Division</span>
              </div>

              {/* Stat badges */}
              <div className="grid grid-cols-3 gap-3 bg-slate-50 border border-slate-200 p-4 rounded-xl text-center mt-1">
                <div className="flex flex-col gap-0.5 border-r border-slate-200">
                  ...
                  <span className="text-sm font-extrabold text-slate-800">{model.age} yrs</span>
                </div>
                <div className="flex flex-col gap-0.5 border-r border-slate-200">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Height</span>
                  <span className="text-sm font-extrabold text-slate-800">{model.height} cm</span>
                </div>
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Weight</span>
                  <span className="text-sm font-extrabold text-slate-800">{model.weight} kg</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col gap-5">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div className="flex flex-col gap-0.5">
                <h3 className="text-base font-bold text-slate-900">Model Portfolio</h3>
                <span className="text-xs text-slate-400">Multiple photos and studio registers ({model.images?.length || 1} images)</span>
              </div>
            ...
            {model.images && model.images.length > 0 ? (
              viewMode === 'grid' ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {model.images.map((img, index) => (
                    <div
                      ...
                      className="group relative aspect-[3/4] rounded-xl overflow-hidden border border-slate-200 bg-slate-100 cursor-pointer shadow-sm hover:border-accent-400 hover:shadow-md transition-all duration-200"
                    >
                      <img
                        src={img}
                      ...
                      <div className="absolute inset-0 bg-black/25 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <span className="px-3 py-1.5 bg-white/95 backdrop-blur-sm rounded-lg text-xs font-bold text-slate-800 shadow-lg flex items-center gap-1.5">
                ...
                <div className="flex flex-col gap-6">
                  {model.images.map((img, index) => (
                    <div
                      key={index}
                      className="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden shadow-sm flex flex-col"
                    >
                      <div className="relative aspect-[16/10] w-full">
                        ...
                        <button
                          onClick={() => setActiveImageIndex(index)}
                          className="absolute bottom-4 right-4 px-3 py-1.5 bg-white/95 backdrop-blur-sm hover:bg-white rounded-lg text-xs font-bold text-slate-800 shadow-md flex items-center gap-1.5 transition-colors border border-slate-200"
                        >
                          <svg className="w-3.5 h-3.5 text-accent-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                      ...
                      <div className="p-4 border-t border-slate-200 bg-white flex justify-between items-center">
                        <span className="text-xs font-bold text-slate-500 uppercase tracking-wide">Studio Image #{index + 1}</span>
                        <span className="text-xs text-slate-400 font-medium">Dimension Aspect 4:5</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="py-8 text-center text-xs text-slate-400">No images added in portfolio.</div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col gap-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Contact & Registration</h3>
            <div className="flex flex-col gap-3.5 text-xs text-slate-700">
              <div className="flex flex-col gap-1 border-b border-slate-100 pb-2.5">
                ...
                <span className="font-bold text-slate-900 truncate">{model.email}</span>
              </div>
              <div className="flex flex-col gap-1 border-b border-slate-100 pb-2.5">
                <span className="text-[10px] text-slate-400 uppercase font-semibold">Phone Number</span>
                <span className="font-bold text-slate-900">{model.phone}</span>
              </div>
              <div className="flex flex-col gap-1 border-b border-slate-100 pb-2.5">
                <span className="text-[10px] text-slate-400 uppercase font-semibold">Gender Identity</span>
                <span className="font-bold text-slate-900">{model.gender}</span>
              </div>
              <div className="flex flex-col gap-1 border-b border-slate-100 pb-2.5">
                <span className="text-[10px] text-slate-400 uppercase font-semibold">Location</span>
                <span className="font-bold text-slate-900">{model.city}, {model.state}, {model.country}</span>
              </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col gap-4">
            ...
            <p className="text-xs text-slate-600 leading-relaxed bg-slate-50 border border-slate-150 p-4 rounded-xl">
